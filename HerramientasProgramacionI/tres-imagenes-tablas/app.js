/* ============================================================
   CLASE 3 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Imágenes, multimedia, tablas y listas)
   ============================================================ */

const STORAGE_KEY = 'curso-imagenes-tablas';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', 'Imágenes y alt', 'Figure y figcaption', 'Tablas', 'Descanso', 'Video y audio', 'Galería y accesibilidad', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '🖼️ Ilustrador',
  2: '🖼️ Curador',
...
const TALLER_ORDER = {
  2: ['2table', '2thead', '2tbody', '2tr', '2td']
};
const TALLER_INPUT = { 3: 'alt' };
const TALLER_MSGS = {
  1: '¡Perfecto! img=imagen, alt=texto alternativo, table=tabla, th=encabezado.',
  2: '¡Correcto! table → thead → tbody → tr → td.',
  3: '¡Bien! El atributo que falta es "alt".',
  4: '🏆 ¡JEFE FINAL VENCIDO! Toda imagen informativa necesita su atributo alt.'
};
const TALLER_HINTS = {
  1: 'Pista: img=imagen, alt=texto alternativo, table=tabla, th=encabezado.',
  2: 'Pista: la tabla va afuera, luego encabezado, cuerpo, fila y celda.',
  3: 'Pista: el atributo es "alt" (texto alternativo).',
  4: 'Pista: falta el atributo alt.'
};
...
const IMG_MEANINGS = {
  gato: '🐱 "Un gato naranja durmiendo" — tiene alt, se lee bien.',
  logo: '🏫 "Logo del colegio" — tiene alt, se lee bien.',
  grafico: '📈 (silencio) — no tiene alt, el lector no dice nada.',
  foto: '📷 (silencio) — no tiene alt, el lector no dice nada.'
};

function inicializarImagenes() {
  const tagEl = document.getElementById('img-tag');
  const meaningEl = document.getElementById('img-meaning');
  const fbTxt = document.getElementById('img-feedback-txt');
  if (!tagEl) return;
  document.querySelectorAll('[data-img]').forEach(btn => {
    btn.addEventListener('click', () => {
      const img = btn.dataset.img;
      tagEl.textContent = img + '.jpg';
      meaningEl.textContent = IMG_MEANINGS[img];
      fbTxt.textContent = IMG_MEANINGS[img];
      addXP(1);
    });
  });
}
...
const TAB_MEANINGS = {
  table: 'La tabla completa: el contenedor de todo.',
  thead: 'El encabezado: la fila con los títulos de las columnas.',
  tbody: 'El cuerpo: donde van los datos.',
  tr: 'Una fila (table row): una línea horizontal de celdas.',
  th: 'Una celda de encabezado (table header): un título de columna.',
  td: 'Una celda de dato (table data): un valor dentro de la tabla.'
};

function inicializarTablas() {
  const tagEl = document.getElementById('tab-tag');
  const meaningEl = document.getElementById('tab-meaning');
  const fbTxt = document.getElementById('tab-feedback-txt');
  if (!tagEl) return;
  document.querySelectorAll('[data-tab]').forEach(btn => {
    btn.addEventListener('click', () => {
      const tab = btn.dataset.tab;
      tagEl.textContent = '<' + tab + '>';
      meaningEl.textContent = TAB_MEANINGS[tab];
      fbTxt.textContent = TAB_MEANINGS[tab];
      addXP(1);
    });
  });
}
...
function inicializarRevisor() {
  ...
  if (!codeEl) return;
  const casos = [
    { code: '&lt;img src="perro.jpg" alt="imagen de un perro"&gt;', bien: false, msg: '❌ Error: "imagen de un perro" repite "imagen de". Mejor: "Un perro marrón jugando en el parque".' },
    { code: '&lt;img src="gato.jpg" alt="Un gato naranja durmiendo"&gt;', bien: true, msg: '✅ Correcto: describe qué hay en la imagen sin repetir "imagen de".' },
    { code: '&lt;img src="grafico-ventas.png"&gt;', bien: false, msg: '❌ Error: falta el atributo alt. La imagen no es accesible.' }
  ];
  let idx = 0;
  const mostrar = () => {
  ...
  document.getElementById('revisor-bien').addEventListener('click', () => {
    ...
    fbTxt.textContent = casos[idx].bien ? '✅ ¡Correcto! Detectaste bien.' : '❌ Te equivocaste: este alt SÍ tiene un error. ' + casos[idx].msg;
    addXP(1);
  ...
  document.getElementById('revisor-mal').addEventListener('click', () => {
    fb.style.display = 'block';
    fbTxt.textContent = !casos[idx].bien ? '✅ ¡Correcto! Detectaste el error. ' + casos[idx].msg : '❌ Te equivocaste: este alt está bien. ' + casos[idx].msg;
    addXP(1);
    idx = (idx + 1) % casos.length;
    setTimeout(mostrar, 2500);
  });
