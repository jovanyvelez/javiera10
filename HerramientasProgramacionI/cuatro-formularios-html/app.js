/* ============================================================
   CLASE 4 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Formularios HTML y accesibilidad básica)
   ============================================================ */

const STORAGE_KEY = 'curso-formularios-html';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', 'La etiqueta form', 'Tipos de input', 'label y accesibilidad', 'Descanso', 'Formulario de contacto', 'Formulario de tareas', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '📝 Formulador',
  2: '⌨️ Tipógrafo',
...
const TALLER_ORDER = {
  2: ['2form', '2label', '2input', '2button']
};
const TALLER_INPUT = { 3: 'email' };
const TALLER_MSGS = {
  1: '¡Perfecto! required=obligatorio, placeholder=ejemplo, name=identifica, value=valor inicial.',
  labels: '¡Muy bien! Nombre=text, correo=email, mensaje=textarea.',
  2: '¡Correcto! El formulario va: form, label, input, button.',
  3: '¡Bien! El correo usa type="email".',
  4: '🏆 ¡JEFE FINAL VENCIDO! Sin label es inaccesible, con for es accesible, y sin type el botón puede enviar por error.'
};
const TALLER_HINTS = {
  1: 'Pista: required=obligatorio, placeholder=ejemplo, name=identifica, value=valor inicial.',
  labels: 'Pista: nombre=text, correo=email, mensaje=textarea.',
  2: 'Pista: form, label, input, button.',
  3: 'Pista: el tipo es "email".',
  4: 'Pista: sin label es inaccesible, con for es accesible, sin type envía por error.'
};
const TALLER_XP = { 1: 25, labels: 25, 2: 30, 3: 30, 4: 55 };
...
const FORM_MEANINGS = {
  form: 'Contenedor de todos los campos del formulario.',
  label: 'Texto que describe cada campo. Se relaciona con el input.',
  input: 'Campo de entrada: texto, correo, contraseña, etc.',
  textarea: 'Área de texto grande, para mensajes de varias líneas.',
  select: 'Menú desplegable con opciones para elegir.',
  button: 'Botón para enviar el formulario o ejecutar una acción.'
};

function inicializarForm() {
  const tagEl = document.getElementById('form-tag');
  const meaningEl = document.getElementById('form-meaning');
  const fbTxt = document.getElementById('form-feedback-txt');
  if (!tagEl) return;
  document.querySelectorAll('[data-form]').forEach(btn => {
    btn.addEventListener('click', () => {
      const form = btn.dataset.form;
      tagEl.textContent = '<' + form + '>';
      meaningEl.textContent = FORM_MEANINGS[form];
      fbTxt.textContent = FORM_MEANINGS[form];
      addXP(1);
    });
  });
}
...
const TIPO_MEANINGS = {
  text: 'Texto libre de una línea. El más común.',
  email: 'Correo electrónico. El navegador valida el formato.',
  password: 'Contraseña. Oculta los caracteres con puntos.',
  number: 'Solo números. Muestra flechas para subir/bajar.',
  date: 'Fecha. Abre un calendario para elegir.',
  checkbox: 'Casilla de verificación. Permite marcar varias.',
  radio: 'Botón de opción. Permite elegir solo una del grupo.'
};
