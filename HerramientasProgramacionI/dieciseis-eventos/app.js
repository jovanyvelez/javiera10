/* ============================================================
   CLASE 16 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Eventos: addEventListener, click, input, submit, delegación)
   ============================================================ */

const STORAGE_KEY = 'curso-eventos';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', '¿Qué es un evento?', 'addEventListener y click', 'input y change', 'Descanso', 'submit y preventDefault', 'Delegación de eventos', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '⚡ Detector',
  2: '🖱️ Clicador',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '❌ Casi. DOM es Document Object Model. ¡A seguir!';
      }
    });
  });
}
...
const TALLER_INPUT = { 3: 'input' };
const TALLER_MSGS = {
  1: '¡Perfecto! click=clic, input=tecla, submit=envío.',
  2: '¡Correcto! Primero seleccionas, luego addEventListener, el evento y la función.',
  3: '¡Bien! El evento que se dispara en cada tecla es "input".',
  4: '🏆 ¡JEFE FINAL VENCIDO! Falta evento.preventDefault() para evitar la recarga.'
};
const TALLER_HINTS = {
  1: 'Pista: click=clic, input=tecla, submit=envío.',
  2: 'Pista: primero seleccionas el elemento, luego escuchas el evento.',
  3: 'Pista: el evento es "input" (se dispara en cada tecla).',
  4: 'Pista: falta llamar a evento.preventDefault().'
};
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🛠️ Maestro de Eventos Completado');
    }
  } else {
...
function inicializarDetector() {
  const tipoEl = document.getElementById('ev-tipo');
  const elemEl = document.getElementById('ev-elemento');
  const fbTxt = document.getElementById('ev-feedback-txt');
  if (!tipoEl) return;
  const registrar = (tipo, elem) => {
    tipoEl.textContent = tipo;
    elemEl.textContent = elem;
    fbTxt.textContent = `Se disparó el evento "${tipo}" en ${elem}.`;
    addXP(1);
  };
  const boton = document.getElementById('ev-boton');
  const input = document.getElementById('ev-input');
  if (boton) boton.addEventListener('click', () => registrar('click', 'el botón'));
  if (input) {
    input.addEventListener('input', () => registrar('input', 'el campo de texto'));
    input.addEventListener('change', () => registrar('change', 'el campo de texto (salió del foco)'));
  }
}

function inicializarContador() {
  const numEl = document.getElementById('cont-num');
  const evEl = document.getElementById('cont-evento');
  const fbTxt = document.getElementById('cont-feedback-txt');
  if (!numEl) return;
  let contador = 0;
  const boton = document.getElementById('cont-boton');
  const reset = document.getElementById('cont-reset');
  boton.addEventListener('click', () => {
    contador++;
    numEl.textContent = contador;
    evEl.textContent = 'click';
    fbTxt.textContent = `¡Clic #${contador}! El evento "click" disparó la función callback.`;
    addXP(1);
  });
  reset.addEventListener('click', () => {
    ...
    numEl.textContent = '0';
    evEl.textContent = '—';
    fbTxt.textContent = 'Contador reiniciado. Vuelve a hacer clic.';
  });
}

function inicializarInput() {
  const campo = document.getElementById('inp-campo');
  const espejo = document.getElementById('inp-espejo');
  const carEl = document.getElementById('inp-caracteres');
  const evEl = document.getElementById('inp-evento');
  if (!campo) return;
  campo.addEventListener('input', () => {
    const valor = campo.value;
    espejo.textContent = valor || 'Tu texto aparecerá aquí en vivo...';
    carEl.textContent = valor.length;
    evEl.textContent = 'input';
    addXP(1);
  });
  campo.addEventListener('change', () => {
    evEl.textContent = 'change';
  });
}
...
function inicializarFormulario() {
  ...
  const recargasEl = document.getElementById('form-recargas');
  const fbTxt = document.getElementById('form-feedback-txt');
  if (!form) return;
  let recargas = 0;
  form.addEventListener('submit', (evento) => {
    ...
    recargasEl.textContent = recargas;
    if (nombre) {
      estadoEl.textContent = 'Enviado';
      fbTxt.textContent = `¡Hola, ${nombre}! El formulario se envió SIN recargar la página (preventDefault).`;
    } else {
      estadoEl.textContent = 'Vacío';
      fbTxt.textContent = 'Escribe tu nombre antes de enviar. La página tampoco se recargó.';
    }
    addXP(2);
  });
}
...
function inicializarDelegacion() {
  const contenedor = document.getElementById('del-contenedor');
  const agregar = document.getElementById('del-agregar');
  const reset = document.getElementById('del-reset');
  ...
  const crearBoton = () => {
    ...
    b.className = 'btn';
    b.textContent = `Botón ${n}`;
    contenedor.appendChild(b);
  };
  agregar.addEventListener('click', () => {
    crearBoton();
    fbTxt.textContent = `Se creó el "Botón ${n}". El escuchador del contenedor lo capturará al hacer clic.`;
    addXP(1);
  });
  reset.addEventListener('click', () => {
    contenedor.innerHTML = '';
    n = 0;
    fbTxt.textContent = 'Contenedor vacío. Agrega botones de nuevo.';
  });
  contenedor.addEventListener('click', (evento) => {
    if (evento.target.tagName === 'BUTTON') {
      fbTxt.textContent = `Delegación: hiciste clic en "${evento.target.textContent}" (detectado por evento.target).`;
      addXP(1);
    }
  });
}
