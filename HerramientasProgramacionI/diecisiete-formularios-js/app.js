/* ============================================================
   CLASE 17 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Formularios con JavaScript y checkpoint de JS)
   ============================================================ */

const STORAGE_KEY = 'curso-formularios-js';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', 'Capturar datos', 'FormData', 'Validación básica', 'Descanso', 'Agregar a un array', 'Renderizar la lista', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '📥 Capturador',
  2: '📦 FormData',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '✅ ¡Correcto! push() agrega un elemento al final del array. ¡Volvamos!';
      } else {
        op.classList.add('incorrecta');
        result.className = 'trivia-result visible';
        result.style.background = 'rgba(244, 63, 94, 0.12)';
        result.style.color = 'var(--rosa)';
        result.textContent = '❌ Casi. El método es push(). ¡A seguir!';
      }
    });
  });
}
...
const TALLER_ORDER = {
  2: ['2capturar', '2validar', '2objeto', '2push', '2render']
};
const TALLER_INPUT = { 3: 'push' };
const TALLER_MSGS = {
  1: '¡Perfecto! .value=leer, FormData=recoger, push()=agregar, innerHTML=pintar.',
  2: '¡Excelente! capturar → validar → objeto → push() → renderizar.',
  3: '¡Bien! El método es push().',
  4: '🏆 ¡JEFE FINAL VENCIDO! Debe usar === (comparación), no = (asignación).'
};
const TALLER_HINTS = {
  1: 'Pista: .value lee, FormData recoge, push() agrega, innerHTML pinta.',
  2: 'Pista: primero capturas, luego validas, creas el objeto, lo agregas y lo muestras.',
  3: 'Pista: el método es "push".',
  4: 'Pista: hay un = que debería ser ===.'
};
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🛠️ Maestro de Formularios Completado');
    }
  } else {
...
function inicializarCaptura() {
  ...
  input.addEventListener('input', () => {
    ...
    output.textContent = val === '' ? '—' : val;
    count.textContent = val.length;
    if (val.length > 0) addXP(1);
  });
}
...
function inicializarFormData() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const cells = document.querySelectorAll('#fd-output .state-cell .sval');
    if (cells[0]) cells[0].textContent = nombre;
...
function inicializarValidacion() {
  ...
  const fb = document.getElementById('val-feedback');
  const fbTxt = document.getElementById('val-feedback-txt');
  if (!form) return;
  form.addEventListener('submit', (e) => {
    e.preventDefault();
    const titulo = document.getElementById('val-titulo').value.trim();
    ...
    if (titulo === '') {
      ...
      msg = '❌ El correo no es válido (falta el @).';
    } else {
      msg = '✅ ¡Todo correcto! La tarea pasó la validación.';
    }
...
function inicializarTareas() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (titulo === '') {
      mostrarToast('⚠️ El título no puede estar vacío');
      return;
    }
    const tarea = { titulo, descripcion: desc, categoria: cat };
    tareas.push(tarea);
    document.getElementById('tarea-titulo').value = '';
    document.getElementById('tarea-desc').value = '';
    document.getElementById('tarea-count').textContent = tareas.length;
    document.getElementById('tarea-ultima').textContent = titulo;
    document.getElementById('tarea-json').textContent = JSON.stringify(tareas, null, 2);
    addXP(3);
  });
}
...
function inicializarRender() {
  ...
  const limpiar = document.getElementById('render-limpiar');
  const lista = document.getElementById('tarea-lista');
  const vacio = document.getElementById('render-vacio');
  if (!btn) return;
  ...
  function renderizar() {
    ...
    let html = '';
    tareas.forEach((t, i) => {
      html += `<div style="background:var(--fondo-claro);border:1px solid var(--tarjeta-borde);border-radius:10px;padding:0.8rem 1rem;">
        <strong style="color:var(--cian);">${i + 1}. ${t.titulo}</strong>
        <span style="color:var(--ambar);margin-left:0.5rem;font-size:0.8rem;">[${t.categoria}]</span>
        <div style="color:var(--texto-suave);font-size:0.85rem;">${t.descripcion || '—'}</div>
      </div>`;
    });
  ...
  limpiar.addEventListener('click', () => {
    tareas.length = 0;
    document.getElementById('tarea-count').textContent = '0';
    document.getElementById('tarea-ultima').textContent = '—';
    document.getElementById('tarea-json').textContent = '[]';
    renderizar();
  });
}
