/* ============================================================
   CLASE 18 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Persistencia con localStorage y JSON)
   ============================================================ */

const STORAGE_KEY = 'curso-localstorage-json';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', '¿Qué es localStorage?', 'setItem y getItem', 'JSON.stringify y parse', 'Descanso', 'removeItem y clear', 'Limitaciones y seguridad', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '💾 Persistente',
  2: '📥 Guardián',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '✅ ¡Correcto! JSON = JavaScript Object Notation (Notación de Objetos de JavaScript). ¡Volvamos!';
      } else {
        op.classList.add('incorrecta');
        result.className = 'trivia-result visible';
        result.style.background = 'rgba(244, 63, 94, 0.12)';
        result.style.color = 'var(--rosa)';
        result.textContent = '❌ Casi. JSON es JavaScript Object Notation. ¡A seguir!';
      }
    });
  });
}
...
const TALLER_ORDER = {
  2: ['2stringify', '2set', '2get', '2parse']
};
const TALLER_INPUT = { 3: 'stringify' };
const TALLER_MSGS = {
  1: '¡Perfecto! setItem=guardar, getItem=leer, removeItem=borrar.',
  2: '¡Excelente! stringify → setItem → getItem → parse.',
  3: '¡Bien! Se usa JSON.stringify para convertir el array a texto.',
  4: '🏆 ¡JEFE FINAL VENCIDO! Nunca guardes contraseñas ni tarjetas en localStorage.'
};
const TALLER_HINTS = {
  1: 'Pista: setItem guarda, getItem lee, removeItem borra.',
  2: 'Pista: primero conviertes a texto, luego guardas, luego lees y conviertes de vuelta.',
  3: 'Pista: la función es "stringify" (de JSON.stringify).',
  4: 'Pista: hay datos sensibles guardados.'
};
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🛠️ Guardián de Datos Completado');
    }
  } else {
...
/* Módulo 1: pizarra (memoria) vs cuaderno (localStorage) */
function inicializarMemoria() {
  const memVal = document.getElementById('mem-val');
  const lsVal = document.getElementById('ls-val');
  const fbTxt = document.getElementById('mem-feedback-txt');
  if (!memVal) return;

  let memoria = null;
  const LS_DEMO = 'demo-memoria';

  const render = () => {
    memVal.textContent = memoria === null ? '—' : memoria;
    const ls = localStorage.getItem(LS_DEMO);
    lsVal.textContent = ls === null ? '—' : ls;
  };

  document.getElementById('mem-escribir').addEventListener('click', () => {
    memoria = 'Hola, soy volátil';
    fbTxt.textContent = 'Escribiste en la pizarra (memoria). Ahora toca "Recargar" y verás que se pierde.';
    addXP(1);
    render();
  });
  document.getElementById('ls-escribir').addEventListener('click', () => {
    localStorage.setItem(LS_DEMO, 'Hola, soy persistente');
    fbTxt.textContent = 'Escribiste en el cuaderno (localStorage). Toca "Recargar" y verás que sobrevive.';
    addXP(1);
    render();
  });
  document.getElementById('mem-reload').addEventListener('click', () => {
    memoria = null;
    fbTxt.textContent = 'Recargaste: la pizarra se borró, pero el cuaderno (localStorage) sigue intacto.';
    addXP(1);
    render();
  });
  render();
...
/* Módulo 2: almacén localStorage en vivo */
function inicializarAlmacen() {
  const claveEl = document.getElementById('alm-clave');
  const valorEl = document.getElementById('alm-valor');
  const input = document.getElementById('alm-input');
  const fbTxt = document.getElementById('alm-feedback-txt');
  if (!valorEl) return;

  const LS_DEMO = 'demo-tarea';

  const render = () => {
    const v = localStorage.getItem(LS_DEMO);
    valorEl.textContent = v === null ? '(vacío)' : v;
  };

  document.getElementById('alm-guardar').addEventListener('click', () => {
    const texto = input.value.trim();
    if (!texto) {
      fbTxt.textContent = 'Escribe algo primero en el campo de texto.';
      return;
    }
    localStorage.setItem(LS_DEMO, texto);
    fbTxt.textContent = `Guardaste "${texto}" con setItem. Recarga la página y seguirá aquí.`;
    addXP(2);
    render();
  });
  document.getElementById('alm-leer').addEventListener('click', () => {
    const v = localStorage.getItem(LS_DEMO);
    fbTxt.textContent = v === null
      ? 'getItem devolvió null: no hay nada guardado todavía.'
      : `getItem devolvió: "${v}"`;
    addXP(1);
    render();
  });
  document.getElementById('alm-limpiar').addEventListener('click', () => {
    localStorage.removeItem(LS_DEMO);
    input.value = '';
    fbTxt.textContent = 'Limpiaste la demo con removeItem.';
    render();
  });
  render();
}

/* Módulo 3: traductor JSON */
function inicializarJson() {
  const objEl = document.getElementById('json-obj');
  const txtEl = document.getElementById('json-txt');
  const fbTxt = document.getElementById('json-feedback-txt');
  if (!txtEl) return;

  const objeto = { nombre: 'Javiera', edad: 15 };
  let texto = null;

  document.getElementById('json-stringify').addEventListener('click', () => {
    texto = JSON.stringify(objeto);
    txtEl.textContent = texto;
    fbTxt.textContent = `stringify convirtió el objeto en texto: ${texto}`;
    addXP(2);
  });
  document.getElementById('json-parse').addEventListener('click', () => {
    if (texto === null) {
      fbTxt.textContent = 'Primero toca "stringify" para generar el texto JSON.';
      return;
    }
    const deVuelta = JSON.parse(texto);
    fbTxt.textContent = `parse convirtió el texto de vuelta en objeto: nombre=${deVuelta.nombre}, edad=${deVuelta.edad}`;
    addXP(2);
  });
}

/* Módulo 5: borrar con cuidado */
function inicializarBorrar() {
  const aEl = document.getElementById('bor-a');
  const bEl = document.getElementById('bor-b');
  const fbTxt = document.getElementById('bor-feedback-txt');
  if (!aEl) return;

  const render = () => {
    aEl.textContent = localStorage.getItem('demo-a') || '—';
    bEl.textContent = localStorage.getItem('demo-b') || '—';
  };

  document.getElementById('bor-guardar').addEventListener('click', () => {
    localStorage.setItem('demo-a', 'valor A');
    localStorage.setItem('demo-b', 'valor B');
    fbTxt.textContent = 'Guardaste dos claves: demo-a y demo-b.';
    addXP(1);
    render();
  });
  document.getElementById('bor-remove-a').addEventListener('click', () => {
    localStorage.removeItem('demo-a');
    fbTxt.textContent = 'removeItem("demo-a") borró solo esa clave. demo-b sigue intacta.';
    addXP(1);
    render();
  });
  document.getElementById('bor-remove-b').addEventListener('click', () => {
    localStorage.removeItem('demo-b');
    fbTxt.textContent = 'removeItem("demo-b") borró solo esa clave. demo-a sigue intacta.';
    addXP(1);
    render();
  });
  document.getElementById('bor-clear').addEventListener('click', () => {
    localStorage.removeItem('demo-a');
    localStorage.removeItem('demo-b');
    fbTxt.textContent = 'clear() vació todo el almacén de la demo. (Aquí simulamos clear borrando ambas claves para no afectar tu progreso).';
    addXP(2);
    render();
  });
  render();
