/* ============================================================
   CLASE 9-TALLER — HERRAMIENTAS DE PROGRAMACIÓN I
   (Taller guiado: Responsive Design y Mobile First)
   ============================================================ */

const STORAGE_KEY = 'curso-taller-responsive-design';
const TOTAL_MODULOS = 6;
const XP_TOTAL = 500;

const LABELS_MODULOS = ['Inicio', 'El sitio base', 'Mobile First y viewport', 'Tabla → cards', 'Menú adaptable', 'Taller final'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '🧩 Diagnosticador',
  2: '📲 Mobile First',
...
const estado = {
  ...
  badges: new Set(),
  xp: 0,
  // Entrega en el repositorio (módulo 5)
  pasos: new Set(),
  autoeval: new Set(),
  usuarioGithub: '',
  publicado: false
...
const TALLER_INPUT = { 3: 'none' };
const TALLER_MSGS = {
  1: '¡Perfecto! viewport=etiqueta, media query=condición, Mobile First=estrategia.',
  2: '¡Correcto! La escalera va: base móvil → tablet → escritorio.',
  3: '¡Bien! Para ocultar la fila de títulos se usa display: none.',
  4: '🏆 ¡JEFE FINAL VENCIDO! Los píxeles fijos rompen el responsive; usa rem o %.'
};
const TALLER_HINTS = {
  1: 'Pista: viewport=etiqueta, media query=condición, Mobile First=estrategia.',
  2: 'Pista: empieza por móvil, luego tablet, luego escritorio.',
  3: 'Pista: la palabra es "none".',
  4: 'Pista: 16px y 10px son tamaños fijos.'
};
...
/* ============================================================
   ENTREGA EN EL REPOSITORIO (módulo 5)
   Mismo repositorio creado en la clase 1: claseNN/
   ============================================================ */
const TOTAL_PASOS = 5;
...
const CARPETA_CLASE = 'clase09-taller';
...
function generarEnlaces(silencioso) {
  ...
  salida.innerHTML = `
    <div class="url-fila">
      <span class="url-etiqueta">📄 El taller de hoy</span>
  ...
  if (final) {
    final.placeholder = base + CARPETA_CLASE + '/';
    if (!final.value) final.value = base + CARPETA_CLASE + '/';
  }
...
function verificarPublicacion() {
  ...
  if (!m) {
    fallo(`❌ Esa dirección no parece de GitHub Pages. Debe tener la forma <code>https://tu-usuario.github.io/${REPO_NOMBRE}/${CARPETA_CLASE}/</code>.`);
    return;
  }
  ...
  if (!resto.includes(CARPETA_CLASE)) {
    extra += `<br>💡 Esa es tu portada. Comprueba también <code>${CARPETA_CLASE}/</code>, que es la entrega de hoy.`;
  }
...
function inicializarSimuladores() {
  inicializarProbadorAncho();
}

const DEMO_FEEDBACK = {
  movil: 'En móvil (320px) la tabla se convierte en cards apiladas y el menú se encoge. Así debe quedar tu sitio.',
  tablet: 'En tablet (600px) la tabla aún se ve como tabla, pero el menú ya tiene un tamaño intermedio.',
  escritorio: 'En escritorio (900px) todo cabe cómodo: tabla completa y menú con aire. Es el punto de partida.'
};

function inicializarProbadorAncho() {
  const sitios = document.querySelectorAll('.demo-sitio');
  if (!sitios.length) return;
  const fb = document.getElementById('demo-feedback');

  document.querySelectorAll('[data-demo]').forEach(btn => {
    btn.addEventListener('click', () => {
      const modo = btn.dataset.demo;
      sitios.forEach(s => {
        s.classList.remove('movil', 'tablet', 'escritorio');
        s.classList.add(modo);
      });
      if (fb) fb.textContent = DEMO_FEEDBACK[modo] || '';
      addXP(1);
    });
  });
}
