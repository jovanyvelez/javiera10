/* ============================================================
   CLASE 15 — HERRAMIENTAS DE PROGRAMACIÓN I
   (DOM: seleccionar y modificar elementos)
   ============================================================ */

const STORAGE_KEY = 'curso-dom-seleccion';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', '¿Qué es el DOM?', 'Seleccionar elementos', 'Modificar contenido', 'Descanso', 'Clases y atributos', 'Crear elementos', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '🌳 Explorador',
  2: '🎯 Selector',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '✅ ¡Correcto! DOM = Document Object Model (Modelo de Objetos del Documento). ¡Volvamos!';
      } else {
        op.classList.add('incorrecta');
        result.className = 'trivia-result visible';
        result.style.background = 'rgba(244, 63, 94, 0.12)';
        result.style.color = 'var(--rosa)';
        result.textContent = '❌ Casi. La "D" es de Document (documento). ¡A seguir!';
      }
    });
  });
}
...
const TALLER_ORDER = {
  2: ['2create', '2text', '2append']
};
const TALLER_INPUT = { 3: 'textContent' };
const TALLER_MSGS = {
  1: '¡Perfecto! getElementById=por id, querySelectorAll=todos, createElement=crear.',
  2: '¡Correcto! Primero crear, luego dar contenido, luego agregar.',
  3: '¡Bien! La propiedad es textContent.',
  4: '🏆 ¡JEFE FINAL VENCIDO! Falta el punto: debe ser ".item".'
};
const TALLER_HINTS = {
  1: 'Pista: getElementById=id, querySelectorAll=todos, createElement=crear.',
  2: 'Pista: crear → contenido → agregar.',
  3: 'Pista: la propiedad es "textContent".',
  4: 'Pista: falta el punto antes de "item".'
};
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🛠️ Domador del DOM Completado');
    }
  } else {
...
const DOM_MEANINGS = {
  document: 'La raíz de todo: el objeto que representa la página completa.',
  html: 'El nodo raíz del documento: contiene todo lo demás.',
  body: 'El cuerpo de la página: todo lo visible vive aquí.',
  h1: 'Un encabezado de nivel 1: el título principal.',
  p: 'Un párrafo de texto.',
  ul: 'Una lista desordenada (con viñetas).',
  li: 'Un elemento de lista: cada ítem dentro de ul u ol.'
};

function inicializarArbolDom() {
  const tagEl = document.getElementById('dom-tag');
  const meaningEl = document.getElementById('dom-meaning');
  const fbTxt = document.getElementById('dom-feedback-txt');
  if (!tagEl) return;
  document.querySelectorAll('[data-dom]').forEach(btn => {
    btn.addEventListener('click', () => {
      const dom = btn.dataset.dom;
      tagEl.textContent = dom;
      meaningEl.textContent = DOM_MEANINGS[dom];
      fbTxt.textContent = DOM_MEANINGS[dom];
      addXP(1);
    });
  });
}
...
function inicializarSelector() {
  ...
  const run = document.getElementById('selector-run');
  const fbTxt = document.getElementById('selector-feedback-txt');
  const demo = document.getElementById('selector-demo');
  if (!input || !run) return;
  const buscar = () => {
    const sel = input.value.trim();
    if (!sel) { fbTxt.textContent = 'Escribe un selector CSS primero.'; return; }
    let matches;
    try {
      matches = demo.querySelectorAll(sel);
    } catch (e) {
      fbTxt.textContent = '❌ Ese selector no es válido. Prueba con h2, .item, #titulo o li.';
      return;
    }
    demo.querySelectorAll('.sel-hit').forEach(el => el.classList.remove('sel-hit'));
    matches.forEach(el => {
      el.classList.add('sel-hit');
      el.style.outline = '2px solid var(--verde)';
      el.style.outlineOffset = '2px';
    });
    if (matches.length === 0) {
      fbTxt.textContent = `🔍 "${sel}" no encontró ningún elemento.`;
    } else {
      fbTxt.textContent = `✅ "${sel}" encontró ${matches.length} elemento(s). ¡Mira el resaltado verde!`;
      addXP(2);
    }
  };
...
function inicializarContenido() {
  ...
  const demo = document.getElementById('contenido-demo');
  const fbTxt = document.getElementById('contenido-feedback-txt');
  const btnText = document.getElementById('contenido-text');
  const btnHtml = document.getElementById('contenido-html');
  const btnReset = document.getElementById('contenido-reset');
  if (!input || !demo) return;
  btnText.addEventListener('click', () => {
    demo.textContent = input.value;
    fbTxt.textContent = '🔤 Aplicaste textContent: el texto se mostró literal (las etiquetas se ven como texto).';
    addXP(2);
  });
  btnHtml.addEventListener('click', () => {
    demo.innerHTML = input.value;
    fbTxt.textContent = '🧩 Aplicaste innerHTML: si escribiste etiquetas, se interpretaron como HTML.';
    addXP(2);
  });
  btnReset.addEventListener('click', () => {
    demo.textContent = 'Texto original';
    input.value = '';
    fbTxt.textContent = 'Escribe algo y elige un método para aplicarlo.';
  });
}

function inicializarClases() {
  const demo = document.getElementById('clase-demo');
  const fbTxt = document.getElementById('clase-feedback-txt');
  const reset = document.getElementById('clase-reset');
  if (!demo) return;
  const aplicar = (clase, accion) => {
    if (accion === 'add') demo.classList.add(clase);
    else if (accion === 'remove') demo.classList.remove(clase);
    else if (accion === 'toggle') demo.classList.toggle(clase);
    const clases = demo.className ? demo.className.split(/\s+/).join(', ') : '(ninguna)';
    fbTxt.textContent = `Clases actuales: ${clases}`;
    addXP(1);
  };
  document.querySelectorAll('[data-clase]').forEach(btn => {
    btn.addEventListener('click', () => aplicar(btn.dataset.clase, 'add'));
  });
  document.querySelectorAll('[data-clase-remove]').forEach(btn => {
    btn.addEventListener('click', () => aplicar(btn.dataset.claseRemove, 'remove'));
  });
  document.querySelectorAll('[data-clase-toggle]').forEach(btn => {
    btn.addEventListener('click', () => aplicar(btn.dataset.claseToggle, 'toggle'));
  });
  reset.addEventListener('click', () => {
    demo.className = '';
    demo.style.background = 'var(--fondo-claro)';
    demo.style.border = '2px solid var(--tarjeta-borde)';
    demo.style.color = 'var(--texto)';
    demo.style.opacity = '1';
    fbTxt.textContent = 'Toca un botón para modificar las clases del elemento.';
  });
}

function inicializarListaTareas() {
  const lista = document.getElementById('lista-tareas');
  const agregar = document.getElementById('tarea-agregar');
  const reset = document.getElementById('tarea-reset');
  const fbTxt = document.getElementById('tarea-feedback-txt');
  if (!lista || !agregar) return;
  const tareas = ['Estudiar el DOM', 'Practicar querySelector', 'Crear una lista dinámica', 'Modificar clases', 'Agregar elementos'];
  let idx = 0;
  agregar.addEventListener('click', () => {
    const li = document.createElement('li');
    li.textContent = tareas[idx % tareas.length];
    lista.append(li);
    fbTxt.textContent = `➕ Se creó un &lt;li&gt; nuevo con "${tareas[idx % tareas.length]}" y se agregó a la lista.`;
    idx++;
    addXP(2);
  });
  reset.addEventListener('click', () => {
    lista.innerHTML = '<li>Estudiar el DOM</li>';
    idx = 0;
    fbTxt.textContent = 'Cada clic crea un &lt;li&gt; nuevo y lo agrega a la lista, sin recargar la página.';
  });
}
