/* ============================================================
   CLASE 14 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Objetos literales y métodos de array)
   ============================================================ */

const STORAGE_KEY = 'curso-objetos-metodos-array';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', 'Objetos literales', 'Acceso a propiedades', 'forEach y map', 'Descanso', 'filter, find y some', 'Array de objetos', 'Taller'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '📦 Constructor',
  2: '🔑 Accesor',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '✅ ¡Correcto! map() devuelve un array nuevo transformando cada elemento. ¡Volvamos!';
      } else {
        op.classList.add('incorrecta');
        result.className = 'trivia-result visible';
        result.style.background = 'rgba(244, 63, 94, 0.12)';
        result.style.color = 'var(--rosa)';
        result.textContent = '❌ Casi. Es map(): devuelve un array nuevo transformando cada elemento. ¡A seguir!';
      }
    });
  });
}
...
const TALLER_ORDER = {
  2: ['2crear', '2acceder', '2guardar', '2recorrer']
};
const TALLER_INPUT = { 3: '{}' };
const TALLER_MSGS = {
  1: '¡Perfecto! forEach=recorrer, map=transformar, filter=filtrar, find=buscar.',
  2: '¡Correcto! Crear → acceder → guardar en array → recorrer.',
  3: '¡Bien! Un objeto literal se escribe entre llaves { }.',
  4: '🏆 ¡JEFE FINAL VENCIDO! filter() devuelve un array con las tareas completadas.'
};
const TALLER_HINTS = {
  1: 'Pista: forEach recorre, map transforma, filter filtra, find busca el primero.',
  2: 'Pista: primero creas el objeto, luego accedes, luego guardas varios, luego recorres.',
  3: 'Pista: el objeto literal va entre llaves { y }.',
  4: 'Pista: quieres TODAS las completadas, no solo la primera.'
};
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🛠️ Maestro de Datos Completado');
    }
  } else {
...
function inicializarObjeto() {
  const codeEl = document.getElementById('obj-code');
  if (!codeEl) return;
  const partes = {
    titulo: '  titulo: <span class="str">"Estudiar"</span>,',
    materia: '  materia: <span class="str">"Matemáticas"</span>,',
    completada: '  completada: <span class="kw">false</span>,',
    id: '  id: <span class="num">1</span>,'
  };
  const orden = ['titulo', 'materia', 'completada', 'id'];
  const agregados = [];
  ...
  function renderObjeto() {
    if (agregados.length === 0) {
      codeEl.innerHTML = '<span class="cm">// Toca los botones para construir el objeto</span>';
      return;
    }
    const ordenado = orden.filter(k => agregados.includes(k));
    codeEl.innerHTML = '<span class="kw">const</span> tarea = {\n' + ordenado.map(k => partes[k]).join('\n') + '\n};';
  }
}

function inicializarAcceso() {
  const codeEl = document.getElementById('acc-code');
  const fbTxt = document.getElementById('acc-feedback-txt');
  if (!codeEl) return;
  let prop = null;
  let forma = null;
  document.querySelectorAll('[data-acc]').forEach(btn => {
    btn.addEventListener('click', () => {
      prop = btn.dataset.acc;
      renderAcceso();
      addXP(1);
    });
  });
  document.querySelectorAll('[data-forma]').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  function renderAcceso() {
    if (!prop || !forma) {
      codeEl.innerHTML = '<span class="cm">// Elige una propiedad y una forma de acceso</span>';
      fbTxt.textContent = 'Toca una propiedad y luego una forma de acceso para ver el código.';
      return;
    }
    if (forma === 'punto') {
      codeEl.innerHTML = 'tarea.<span class="fn">' + prop + '</span>;';
      fbTxt.textContent = 'Notación de punto: rápida y legible. Úsala cuando conoces el nombre exacto.';
    } else {
      codeEl.innerHTML = 'tarea[<span class="str">"' + prop + '"</span>];';
      fbTxt.textContent = 'Notación de corchetes: permite usar una variable o un nombre con espacios.';
    }
  }
}

function inicializarArray() {
  const salida = document.getElementById('arr-salida');
  if (!salida) return;
  const entrada = ['Estudiar', 'Leer', 'Practicar'];
  document.querySelectorAll('[data-arr]').forEach(btn => {
    btn.addEventListener('click', () => {
      const metodo = btn.dataset.arr;
      if (metodo === 'forEach') {
        salida.innerHTML = entrada.map(t => 'console.log(<span class="str">"' + t + '"</span>);').join('\n');
      } else {
        salida.innerHTML = '[ ' + entrada.map(t => '<span class="str">"' + t.toUpperCase() + '"</span>').join(', ') + ' ]';
      }
      addXP(2);
    });
  });
...
function inicializarTareas() {
  const salida = document.getElementById('tareas-salida');
  if (!salida) return;
  const tareas = [
    { id: 1, titulo: 'Estudiar', completada: true },
    { id: 2, titulo: 'Leer', completada: false },
    { id: 3, titulo: 'Practicar', completada: true }
  ];
  document.querySelectorAll('[data-tarea]').forEach(btn => {
    btn.addEventListener('click', () => {
      const metodo = btn.dataset.tarea;
      if (metodo === 'filter') {
        const res = tareas.filter(t => t.completada);
        salida.innerHTML = res.map(t => '{ id: <span class="num">' + t.id + '</span>, titulo: <span class="str">"' + t.titulo + '"</span>, completada: <span class="kw">true</span> }').join('\n');
      } else if (metodo === 'find') {
        const res = tareas.find(t => t.id === 2);
        salida.innerHTML = '{ id: <span class="num">' + res.id + '</span>, titulo: <span class="str">"' + res.titulo + '"</span>, completada: <span class="kw">false</span> }';
      } else {
        const res = tareas.some(t => t.completada);
        salida.innerHTML = '<span class="kw">' + res + '</span> <span class="cm">// hay al menos una completada</span>';
      }
      addXP(2);
    });
  });
