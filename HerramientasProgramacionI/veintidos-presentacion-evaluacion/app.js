/* ============================================================
   CLASE 22 — HERRAMIENTAS DE PROGRAMACIÓN I
   (Presentación final, evaluación y cierre del curso)
   ============================================================ */

const STORAGE_KEY = 'curso-presentacion-evaluacion';
const TOTAL_MODULOS = 8;
const XP_TOTAL = 640;

const LABELS_MODULOS = ['Inicio', 'Preparar la demo', 'Presentación 5 min', 'Revisión de código', 'Descanso', 'Pruebas cruzadas', 'Evaluación final', 'Cierre'];
const BADGES_MODULO = {
  0: '🚀 Iniciado',
  1: '🎬 Preparado',
  2: '🎤 Presentador',
  ...
  opts.forEach(op => {
    op.addEventListener('click', () => {
      ...
      if (elegida === correcta) {
        ...
        result.textContent = '✅ ¡Correcto! CRUD = Create, Read, Update, Delete (Crear, Leer, Actualizar, Borrar). ¡Volvamos!';
      } else {
        op.classList.add('incorrecta');
        result.className = 'trivia-result visible';
        result.style.background = 'rgba(244, 63, 94, 0.12)';
        result.style.color = 'var(--rosa)';
        result.textContent = '❌ Casi. CRUD significa Create, Read, Update, Delete. ¡A seguir!';
      }
    });
  });
}
...
/* ---------- TALLER ---------- */
const TALLER_RESP = {
  hallazgos: { 'a': 'A', 'b': 'B', 'c': 'C' },
  1: { '1a': '1A', '1b': '1B', '1c': '1C', '1d': '1D' },
  4: { '4A': '4A' }
};
const TALLER_ORDER = {
  guion: ['saludo', 'app', 'funciones', 'codigo', 'dificultades'],
  2: ['2saludo', '2app', '2funciones', '2codigo', '2dificultades']
};
const TALLER_INPUT = { 3: 'index.html' };
const TALLER_MSGS = {
  hallazgos: '¡Muy bien! Caerse al borrar=bug, guardar datos=bug, colores=mejora.',
  guion: '¡Excelente! Saludo → app → funcionalidades → código → dificultades.',
  1: '¡Perfecto! HTML=10%, CSS=20%, CRUD=20%, localStorage=10%.',
  2: '¡Correcto! El guion va: saludo, app, funcionalidades, código, dificultades.',
  3: '¡Bien! El archivo principal es index.html.',
  4: '🏆 ¡JEFE FINAL VENCIDO! Esa retroalimentación es constructiva: positivo + mejora + ánimo.'
};
const TALLER_HINTS = {
  hallazgos: 'Pista: caerse=bug, guardar=bug, colores=mejora.',
  guion: 'Pista: saludo arriba, dificultades al final.',
  1: 'Pista: HTML=10%, CSS=20%, CRUD=20%, localStorage=10%.',
  2: 'Pista: saludo, app, funcionalidades, código, dificultades.',
  3: 'Pista: el archivo es "index.html".',
  4: 'Pista: tiene positivo, mejora y ánimo.'
};
const TALLER_XP = { hallazgos: 25, guion: 30, 1: 25, 2: 30, 3: 30, 4: 55 };
...
function validarReto(id) {
  ...
  if (allOk) {
    ...
    if (Object.keys(estado.talleres).length === Object.keys(TALLER_MSGS).length) {
      otorgarBadge('🏆 Desarrollador Web Completado');
    }
  } else {
...
function inicializarChecklist() {
  const cont = document.getElementById('checklist-prep');
  const fb = document.getElementById('checklist-fb');
  if (!cont) return;
  const checks = cont.querySelectorAll('input[data-check]');
  let recompensado = false;
  checks.forEach(chk => {
    chk.addEventListener('change', () => {
      const marcados = cont.querySelectorAll('input[data-check]:checked').length;
      if (marcados === checks.length && !recompensado) {
        recompensado = true;
        addXP(30);
        fb.className = 'resultado-ws visible ok';
        fb.innerHTML = '✅ ¡Checklist completo! Estás listo/a para presentar. <strong>+30 XP</strong>';
      } else if (marcados < checks.length) {
        fb.className = 'resultado-ws visible';
        fb.style.background = 'rgba(34, 211, 238, 0.10)';
        fb.style.borderColor = 'var(--cian)';
        fb.style.color = 'var(--cian)';
        fb.textContent = `📋 ${marcados} de ${checks.length} puntos listos.`;
      }
    });
  });
}

function inicializarRubrica() {
  const cont = document.getElementById('rubrica');
  const scoreEl = document.getElementById('rubrica-score');
  const countEl = document.getElementById('rubrica-count');
  const fbTxt = document.getElementById('rubrica-feedback-txt');
  if (!cont) return;
  const checks = cont.querySelectorAll('input[data-rub]');
  let recompensado = false;
  checks.forEach(chk => {
    chk.addEventListener('change', () => {
      let puntaje = 0;
      ...
      checks.forEach(c => {
        if (c.checked) {
          puntaje += parseInt(c.dataset.rub, 10);
          marcados++;
        }
      });
      scoreEl.textContent = puntaje + '%';
      countEl.textContent = marcados + ' / ' + checks.length;
      if (puntaje >= 80) {
        fbTxt.textContent = '🌟 ¡Excelente! Tu proyecto está muy completo.';
      } else if (puntaje >= 50) {
        fbTxt.textContent = '👍 Vas bien. Revisa los criterios sin marcar para subir tu nota.';
      } else {
        fbTxt.textContent = '📈 Aún hay trabajo por hacer. Enfócate en los criterios de mayor peso.';
      }
      if (marcados === checks.length && !recompensado) {
        recompensado = true;
        addXP(30);
        mostrarToast('🎉 ¡Rúbrica completa! +30 XP');
      }
    });
  });
}
...
function inicializarRevisor() {
  ...
  if (!codeEl) return;
  const casos = [
    { code: '"Tu código no sirve para nada."', bien: false, msg: '❌ Es destructiva: critica sin explicación ni propuesta.' },
    { code: '"Me gusta cómo organizaste el HTML. Podrías agrupar los colores en variables."', bien: true, msg: '✅ Es constructiva: específica, amable y con propuesta.' },
    { code: '"Esto está mal."', bien: false, msg: '❌ Es destructiva: vaga y sin propuesta de mejora.' }
  ];
  let idx = 0;
  const mostrar = () => {
  ...
  document.getElementById('revisor-bien').addEventListener('click', () => {
    ...
    fbTxt.textContent = casos[idx].bien ? '✅ ¡Correcto! Es constructiva. ' + casos[idx].msg : '❌ Te equivocaste: este comentario SÍ es destructivo. ' + casos[idx].msg;
    addXP(1);
  ...
  document.getElementById('revisor-mal').addEventListener('click', () => {
    fb.style.display = 'block';
    fbTxt.textContent = !casos[idx].bien ? '✅ ¡Correcto! Es destructiva. ' + casos[idx].msg : '❌ Te equivocaste: este comentario es constructivo. ' + casos[idx].msg;
    addXP(1);
    idx = (idx + 1) % casos.length;
    setTimeout(mostrar, 2500);
  });
